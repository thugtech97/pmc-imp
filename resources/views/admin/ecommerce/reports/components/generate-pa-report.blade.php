    ...
    <table>
        ...
        <tbody class="item-style">   
            @forelse ($headers as $pa)
                @php
                    $bal = (!optional($pa->mrs)->order_number) ? $pa->items->sum('qty_to_order') - $pa->items->sum('qty_ordered') 
                    : $pa->mrs->items->where('promo_id', '!=', 1)->sum('qty_to_order') - $pa->mrs->items->where('promo_id', '!=', 1)->sum('qty_ordered');
                @endphp
                <tr class="item-style">
                    ...
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
                            {{ $pa->approved_at ? \Carbon\Carbon::parse($pa->approved_at)->format('F j, Y h:i A') : 'N/A' }}
                        @else
                            {{ $pa->mrs->approved_at ? \Carbon\Carbon::parse($pa->mrs->approved_at)->format('F j, Y h:i A') : 'N/A' }}
                        @endif
                    </td>
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
                    ...
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
                            {{ $pa->received_at ? Carbon\Carbon::parse($pa->received_at)->format('F j, Y h:i A') : 'N/A' }}
                        @else
                            {{ $pa->mrs->received_at ? Carbon\Carbon::parse($pa->mrs->received_at)->format('F j, Y h:i A') : 'N/A' }}
                        @endif
                    </td>
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
                            ...
                            @endif
                        @else
                            @if($pa->mrs->received_at)
                                @if($bal == 0)
                                    {{ "Completed" }}
                                @else
                                    @php
                                        $receivedAt = Carbon\Carbon::parse($pa->mrs->received_at);
                                        $now = Carbon\Carbon::now();
                                        $days = $receivedAt->diffInDays($now);
                                        $hours = $receivedAt->copy()->addDays($days)->diffInHours($now);
                                    @endphp
                    ...
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
                            {{ $pa->received_at ? $bal : 'N/A' }}
                        @else
                            {{ $pa->mrs->received_at ? $bal : 'N/A' }}
                        @endif
                    </td>
                    <td class="text-align-center">
                        @if (!optional($pa->mrs)->order_number)
