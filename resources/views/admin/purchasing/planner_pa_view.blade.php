...
@section('pagecss')
    ...
    <style>
        ...
        .pa-page-header .breadcrumb {
            background: none; padding: 0; margin-bottom: 6px; font-size: 12px;
        }
        ...
        .pa-page-header h4 {
            font-size: 22px; font-weight: 600; color: var(--pa-text);
        ...
        .pa-status-badge i { font-size: 10px; }
        ...
        .pa-card-header .card-icon {
            ...
            color: var(--pa-primary); font-size: 14px; flex-shrink: 0;
        }
        ...
        .pa-number-display .pa-number-value {
            ...
            color: var(--pa-primary); letter-spacing: 0.5px;
        }
        ...
        .pa-meta-item { }
        .pa-meta-item .meta-label {
            font-size: 10.5px; font-weight: 600; text-transform: uppercase;
            letter-spacing: 0.4px; color: var(--pa-text-light); margin-bottom: 4px;
        }
        .pa-meta-item .meta-value {
            font-size: 13px; font-weight: 500; color: var(--pa-text);
        }
        .pa-meta-item .meta-value.mono {
        ...
        /* Form Controls */
        .pa-label {
            font-size: 12px; font-weight: 600; color: var(--pa-text);
        ...
        .pa-textarea {
            ...
            font-size: 13.5px; color: var(--pa-text);
            background: var(--pa-white);
        ...
        /* Items Table */
        .pa-table-wrapper { border-radius: var(--pa-radius-sm); overflow: auto; border: none; }
        ...
        .pa-table tbody td {
            padding: 8px 10px; color: var(--pa-text);
            border-right: 1px solid var(--pa-border); vertical-align: middle;
        }
        ...
        .row-num {
            ...
            background: var(--pa-primary-light); color: var(--pa-primary);
        ...
        .pa-table .form-control:focus {
            border-color: var(--pa-primary);
            box-shadow: 0 0 0 2px rgba(29,78,216,0.1); outline: none;
        }
        .pa-table .form-control[readonly] {
            background: #f8fafc; color: var(--pa-text-muted); cursor: default;
        }
        ...
        .doc-list li a {
            ...
            border: 1px solid #bfdbfe;
            border-radius: var(--pa-radius-sm);
            color: var(--pa-primary); font-size: 12.5px; font-weight: 500;
            text-decoration: none; transition: all 0.15s;
        }
        .doc-list li a:hover { background: #dbeafe; text-decoration: none; }
        ...
        .pa-select {
            ...
            font-family: 'DM Sans', sans-serif;
            font-size: 13.5px; color: var(--pa-text);
            background: var(--pa-white); outline: none;
        ...
        .btn-pa:disabled { opacity: 0.55; cursor: not-allowed; transform: none; box-shadow: none; }
        ...
        .btn-pa-success:disabled { background: #6ee7b7; color: white; cursor: default; transform: none; }

        .btn-pa-danger    { background: var(--pa-danger); color: white; }
        .btn-pa-danger:hover { background: #b91c1c; color: white; box-shadow: 0 4px 12px rgba(220,38,38,0.25); }

        .btn-pa-secondary {
            background: var(--pa-white); color: var(--pa-text-muted);
            border: 1px solid var(--pa-border-dark);
        }
        .btn-pa-secondary:hover { background: #f8fafc; color: var(--pa-text); border-color: #94a3b8; }

        .btn-pa-warning { background: var(--pa-warning); color: white; }
        .btn-pa-warning:hover { background: #b45309; color: white; box-shadow: 0 4px 12px rgba(217,119,6,0.25); }

        /* Verified/approved tag in button */
        .btn-done {
            display: inline-flex; align-items: center; gap: 7px;
            padding: 9px 18px;
            border-radius: var(--pa-radius-sm);
    ...
    <div class="container-fluid" style="max-width: 1600px;">
        ...
        <div class="pa-page-header d-flex align-items-start justify-content-between">
            ...
            @php
                ...
                if (str_contains(strtolower($status), 'cancel'))        $statusClass = 'status-cancelled';
                elseif (str_contains(strtolower($status), 'approved'))  $statusClass = 'status-approved';
                elseif (str_contains(strtolower($status), 'verif'))     $statusClass = 'status-approved';
                elseif (str_contains(strtolower($status), 'pending'))   $statusClass = 'status-pending';
            @endphp
        ...
        <form id="paForm" method="POST" action="{{ route('pa.update') }}">
                ...
                <div class="col-lg-8">
                    <div class="pa-card">
                        <div class="pa-card-header">
                            ...
                            <div>
                                <h6>PA Timeline</h6>
                                <p>Approval and processing status</p>
                            </div>
                        </div>
                        <div class="pa-card-body">
                            ...
                            <tbody>
                                ...
                                @forelse($paHeader->details as $details)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" name="par_to{{ $details->id }}"
                                                value="{{ $details->par_to }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="number" name="qty_to_order{{ $details->id }}"
                                                value="{{ $details->qty_to_order }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }} required>
                                        </td>
                                        <td>
                                            <input type="text" name="previous_po{{ $details->id }}"
                                                value="{{ $details->previous_po }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="text" name="priority_no{{ $details->id }}"
                                                value="{{ $details->priority_no }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="number" name="qty_per_delivery{{ $details->id }}"
                                                value="{{ $details->qty_per_delivery }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="number" name="number_of_deliveries{{ $details->id }}"
                                                value="{{ $details->number_of_deliveries }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="text" name="cost_code{{ $details->id }}"
                                                value="{{ $details->cost_code }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>
                                        <td>
                                            <input type="text" name="remarks{{ $details->id }}"
                                                value="{{ $details->remarks }}" class="form-control"
                                                {{ $role->name !== 'MCD Planner' ? 'readonly' : '' }}>
                                        </td>

                                        @if ($paHeader->received_at)
                                            <td class="purchaser-col">
                                                <input type="text" name="current_po{{ $details->id }}"
                                                    value="{{ $details->current_po }}" class="form-control"
                                                    {{ $role->name !== 'Purchaser' ? 'readonly' : '' }}>
                                            </td>
                                            <td class="purchaser-col">
                                                <input type="date" name="po_date_released{{ $details->id }}"
                                                    value="{{ $details->po_date_released ? \Carbon\Carbon::parse($details->po_date_released)->format('Y-m-d') : '' }}"
                                                    class="form-control"
                                                    {{ $role->name !== 'Purchaser' ? 'readonly' : '' }}>
                                            </td>
                                            <td class="purchaser-col">
                                                <input type="number" name="qty_ordered{{ $details->id }}"
                                                    data-qty="{{ $details->qty_to_order }}"
                                                    value="{{ $details->qty_ordered }}"
                                                    class="form-control qty_ordered"
                                                    {{ $role->name !== 'Purchaser' ? 'readonly' : '' }}>
                                            </td>
                                        @endif
                                    </tr>
                    ...
                    <div class="pa-card">
                        <div class="pa-card-header">
                            ...
                            <div>
                                ...
                                <p>Attached files for this request</p>
                            </div>
                        </div>
                        <div class="pa-card-body">
                            ...
                            @else
                                <p style="color: var(--pa-text-light); font-size: 13px; margin: 0; font-style: italic;">
            ...
            {{-- Assign To (Purchasing Officer only) --}}
            @if ($role->name === 'Purchasing Officer')
                <div class="pa-card">
                    <div class="pa-card-header">
                        <div class="card-icon"><i class="fa fa-user-o"></i></div>
                        <div>
                            <h6>Assign Purchaser</h6>
                            <p>Select a purchaser to handle this PA</p>
                        </div>
                    </div>
                    <div class="pa-card-body">
                        <div class="row">
            ...
            <div class="pa-action-bar">
                ...
                @if ($role->name == 'MCD Verifier')
                    ...
                    @else
                        <button type="button" id="verifyBtn" class="btn-pa btn-pa-success">
                ...
                @if ($role->name == 'MCD Approver')
                    ...
                    @else
                        <button type="button" id="approveBtn" class="btn-pa btn-pa-success">
                ...
                {{-- Purchasing Officer --}}
                @if ($role->name === 'Purchasing Officer')
                    <button type="button" id="assignBtn" class="btn-pa btn-pa-warning">
                ...
                @if ($role->name === 'Purchaser')
                    ...
                    @else
                        <button type="button" id="receiveBtn" class="btn-pa btn-pa-success">
                ...
                @if ($role->name === 'MCD Planner')
                    <button type="submit" class="btn-pa btn-pa-primary">
                ...
                @if ($role->name == 'MCD Approver' || $role->name === 'Purchasing Officer' || $role->name === 'Purchaser')
                    <button type="button" id="cancelBtn" class="btn-pa btn-pa-danger">
                        <i class="fa fa-times"></i> Cancel PA
                    </button>
                @endif
