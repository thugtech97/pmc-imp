                    ...
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                        <div class="col-md-6 mg-t-15">
                            <form method="POST" action="{{ route('website-settings.update') }}" enctype="multipart/form-data" id="selectForm2" class="parsley-style-1" data-parsley-validate novalidate>
                                ...
                                <div class="form-group {{ $errors->has('contact_us_image') ? 'has-error' : '' }}">
                                    <label class="d-block">Contact Us Image</label>
                                    <div class="custom-file">
                                        <input type="file" class="form-control" id="contact_us_image" name="contact_us_image" >
                                        <span class="text-danger tx-12">{{ $errors->first('contact_us_image') }}</span>
                                    </div>
                                    <p class="tx-10">
                                        Required image dimension: 798px by 1000px <br /> Maximum file size: 100KB <br/> File extension: ICO
                                    </p>
                                    @if(isset($web->contact_us_image))
                                        <div>
                                            <img src="{{$web->contact_us_image}}" height="350" width="auto" id="icon_temp" alt="Website Favicon">  <br />
                                        </div>
                                    @endif
                                </div>
                                            ...
                                            <tbody>
                                                @foreach($paymentOptions as $option)
                                                    <tr>
                                                        ...
                                                        <td>{{$option->branch}}</td>
                                                        <td class="text-right">
                                                            <a href="javascript:void(0)" onclick="edit_bank('{{$option->id}}','{{$option->name}}','{{$option->account_no}}','{{$option->branch}}','{{$option->account_name}}')"><i class="fa fa-edit"></i></a>
                                                            <a href="javascript:void(0)" onclick="delete_bank('{{$option->id}}')"><i class="fa fa-trash"></i></a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
