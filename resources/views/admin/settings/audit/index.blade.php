                    ...
                    <table class="table mg-b-0 table-light table-hover" style="word-break: break-all;">
                        ...
                        <tbody>
							@forelse($logs as $log)
								<tr>
									<td>
										<a href="{{ route('users.show', $log->log_by ?? 1) }}"><strong>{{ ucwords(optional($log->admin)->fullname) }}</strong><br><span class="badge badge-primary">{{ optional($log->admin)->userRole($log->log_by) }}</span></a>
									</td>
