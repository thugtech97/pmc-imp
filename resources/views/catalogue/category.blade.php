		...
		<section id="page-title" class="page-title-nobg">
			...
			<div class="container clearfix">
				...
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active" aria-current="page">{{ucwords(strtolower($category->description))}}</li>
				</ol>
			</div>
					...
					<div class="row">
						<div class="col-md-6">
							<form action="{{ route('catalogue.search') }}" method="POST" role="search">
								...
								<input type="hidden" name="category_name" value="{{ $category->description }}">
						...
						<div class="col-md-6">
							...
							<select name="cat" id="cat" class="form-control" onchange="go_to_category_page($(this).val())">
								...
								@forelse(\App\Models\Ecommerce\ProductCategory::all() as $c)
									<option value="{{$c->id}}">{{ucwords(strtolower($c->description))}}</option>								
								@empty
								@endforelse
							</select>
						</div>
										...
										<a class="grid-item" href="{{ route('product.front.show', $p->slug) }}">
											<div class="grid-inner rounded bgwhite" style="padding: 10px;">
                                                ...
                                                <div class="entry-image rounded gradent">
													@if (isset($small->path))
                                                    	<img src="{{ asset('storage/' . $p->PhotoSmall) }}" alt="Image" class="img-fluid" style="max-height:200px;">
													@else
														<img src="{{ asset('images/no-image.png') }}" alt="Image" class="img-fluid" style="height:200px;">
													@endif
