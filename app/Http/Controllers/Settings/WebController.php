...
use App\Models\{MediaAccounts, Permission, Setting};
use App\Models\Ecommerce\PaymentOption;
...
class WebController extends Controller
{
    ...
    public function edit(Request $request)
    {
        ...
        $paymentOptions = PaymentOption::orderBy('name','asc')->get();

        return view('admin.settings.website.index',compact('web','medias','paymentOptions'));
    }
    ...
    public function update(Request $request)
    {
        $this->validate($request,[
            ...
            //'contact_us_image' => 'required|mimes:jpeg,png,jpg,svg|max:1000|dimensions:width=798,height=1000',
        ]);
        ...
        if($web){
            ...
            if($request->has('contact_us_image')) {
                $this->upload_contact_us_image($request->file('contact_us_image'));
            }
    ...
    public function upload_contact_us_image($image)
    {
        $folder = 'images';
        ...
        $web = Setting::first();
        $web->update(['contact_us_image' => $url]);
    }
    ...
    public function store_payment_option(Request $request)
    {
        ...
        return back()->with([
            'tabname' => 'ecommerce',
            'success' =>  'Payment option has been added.'
        ]);
    }

    public function update_payment_option(Request $request)
    {
        $requestData = $request->all();

        PaymentOption::find($request->id)->update($requestData);

        return back()->with([
            'tabname' => 'ecommerce',
            'success' =>  'Payment option details has been updated.'
        ]);
    }

    public function delete_payment_option(Request $request)
    {
        PaymentOption::find($request->id)->delete();

        return back()->with([
            'tabname' => 'ecommerce',
            'success' =>  'Payment option has been deleted.'
        ]);
    }
